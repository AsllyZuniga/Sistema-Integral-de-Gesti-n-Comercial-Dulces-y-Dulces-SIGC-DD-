...
import { CuotasUploadService } from '../../../../core/services/cuotas-upload.service';

type EstadoCarga = 'idle' | 'cargando' | 'exito' | 'error';

@Component({
  selector: 'app-impacto-categoria-upload',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="cuota-section">
      <h3>Cuotas de impactos por categoría</h3>
      <p class="section-desc">
        Carga un archivo CSV con las cuotas de impactos por categoría para cada vendedor y período.
      </p>
      ...
      <div class="upload-card">
        ...
        >
          ...
          @if (!archivoSeleccionado) {
            ...
            <p class="upload-hint">Formato: <strong>vendedor, períodos de impactos</strong> · Máx. 10 MB</p>
          } @else {
        ...
        @if (estado === 'exito' && resultado) {
          <div class="resultado exito">
            ...
            <div class="resultado-info">
              ...
              @if (resultado.filas_procesadas !== undefined) {
                <p class="resultado-detalle">
                  Filas procesadas: <strong>{{ resultado.filas_procesadas }}</strong> ·
                  Creados: <strong>{{ resultado.registros_creados }}</strong> ·
                  Omitidos: <strong>{{ resultado.registros_omitidos }}</strong>
                </p>
              }
              @if (resultado.errores?.length) {
                <p class="resultado-detalle">Errores: {{ resultado.errores.length }}</p>
              }
            </div>
          </div>
        }
...
export class ImpactoCategoriaUploadComponent {
  ...
  cargar(): void {
    ...
    this.cuotasService.uploadImpactosCategoria(this.archivoSeleccionado).subscribe({
      next: (res: any) => {
        this.estado = 'exito';
        this.resultado = res;
      ...
      error: (err: HttpErrorResponse) => {
        const backendMessage =
          typeof err.error === 'string'
            ? err.error
            : (err.error?.message ?? err.error?.error ?? err.error?.mensaje);
        this.setError(backendMessage || err.message || 'Error al importar impactos');
        this.estado = 'error';
        this.cd.detectChanges();
      },
    });
